"""Normalization pipeline for canonical account and contact tables."""

from __future__ import annotations

from dataclasses import dataclass

from app.bigquery_repository import BigQueryRepository
from app.config import Settings
from app.logging_utils import get_logger


logger = get_logger(__name__)


@dataclass(frozen=True)
class NormalizationPreview:
    """Read-only snapshot of what the pipeline will process."""

    source_contacts: int
    source_business_domains: int
    source_personal_emails: int
    source_distinct_accounts: int


class NormalizationService:
    """Read from contact_master and populate canonical BigQuery tables."""

    def __init__(self, repository: BigQueryRepository, settings: Settings) -> None:
        self.repository = repository
        self.settings = settings

    def preview(self) -> NormalizationPreview:
        """Return a read-only summary of the source table."""

        query = f"""
        SELECT
          COUNT(*) AS source_contacts,
          COUNT(DISTINCT account_key) AS source_distinct_accounts,
          COUNT(DISTINCT IF(is_personal_email, NULL, account_key)) AS source_business_domains,
          COUNTIF(is_personal_email) AS source_personal_emails
        FROM `{self.settings.source_contact_table_fqn}`
        """
        row = self.repository.fetch_one(query)
        return NormalizationPreview(
            source_contacts=int(row.get("source_contacts", 0)),
            source_business_domains=int(row.get("source_business_domains", 0)),
            source_personal_emails=int(row.get("source_personal_emails", 0)),
            source_distinct_accounts=int(row.get("source_distinct_accounts", 0)),
        )

    def normalize(self, dry_run: bool = False) -> None:
        """Run the normalization pipeline or print what would happen."""

        preview = self.preview()
        logger.info(
            "Normalization preview | source=%s | contacts=%s | distinct_accounts=%s | business_domains=%s | personal_emails=%s | dry_run=%s",
            self.settings.source_contact_table_fqn,
            preview.source_contacts,
            preview.source_distinct_accounts,
            preview.source_business_domains,
            preview.source_personal_emails,
            dry_run,
        )

        if dry_run:
            logger.info("Dry run enabled. No BigQuery writes were executed.")
            return

        self._merge_dealer_accounts()
        self._merge_prospect_contacts()
        self._merge_account_relationships()

    def _merge_dealer_accounts(self) -> None:
        """Create or update canonical dealer account rows."""

        query = f"""
        MERGE `{self.settings.dealer_accounts_table_fqn}` AS target
        USING (
          SELECT
            TO_HEX(SHA256(account_key)) AS dealer_account_id,
            account_key,
            email_domain,
            CAST(NULL AS STRING) AS account_name,
            CAST(NULL AS STRING) AS inferred_brand,
            CAST(NULL AS STRING) AS website_url,
            CASE
              WHEN LOGICAL_OR(is_personal_email) THEN 'personal_domain_seed'
              ELSE 'domain_seed'
            END AS account_type,
            'active' AS account_status,
            MAX(confidence_score) AS confidence_score,
            'contact_master_normalization' AS source_type,
            '{self.settings.source_contact_table}' AS source_table,
            LOGICAL_OR(is_personal_email) AS is_personal_domain,
            MIN(observed_at) AS first_seen_at,
            MAX(observed_at) AS last_seen_at,
            CURRENT_TIMESTAMP() AS created_at,
            CURRENT_TIMESTAMP() AS updated_at
          FROM (
            SELECT
              LOWER(TRIM(account_key)) AS account_key,
              LOWER(TRIM(email_domain)) AS email_domain,
              IFNULL(is_personal_email, FALSE) AS is_personal_email,
              IFNULL(confidence_score, 0.0) AS confidence_score,
              COALESCE(last_seen_at, created_at, CURRENT_TIMESTAMP()) AS observed_at
            FROM `{self.settings.source_contact_table_fqn}`
            WHERE account_key IS NOT NULL
              AND TRIM(account_key) != ''
          )
          GROUP BY account_key, email_domain
        ) AS source
        ON target.dealer_account_id = source.dealer_account_id
        WHEN MATCHED THEN
          UPDATE SET
            target.email_domain = COALESCE(target.email_domain, source.email_domain),
            target.account_type = COALESCE(target.account_type, source.account_type),
            target.account_status = COALESCE(source.account_status, target.account_status),
            target.confidence_score = GREATEST(IFNULL(target.confidence_score, 0.0), IFNULL(source.confidence_score, 0.0)),
            target.source_type = source.source_type,
            target.source_table = source.source_table,
            target.is_personal_domain = IFNULL(target.is_personal_domain, FALSE) OR source.is_personal_domain,
            target.first_seen_at = LEAST(IFNULL(target.first_seen_at, source.first_seen_at), source.first_seen_at),
            target.last_seen_at = GREATEST(IFNULL(target.last_seen_at, source.last_seen_at), source.last_seen_at),
            target.updated_at = CURRENT_TIMESTAMP()
        WHEN NOT MATCHED THEN
          INSERT (
            dealer_account_id,
            account_key,
            email_domain,
            account_name,
            inferred_brand,
            website_url,
            account_type,
            account_status,
            confidence_score,
            source_type,
            source_table,
            is_personal_domain,
            first_seen_at,
            last_seen_at,
            created_at,
            updated_at
          )
          VALUES (
            source.dealer_account_id,
            source.account_key,
            source.email_domain,
            source.account_name,
            source.inferred_brand,
            source.website_url,
            source.account_type,
            source.account_status,
            source.confidence_score,
            source.source_type,
            source.source_table,
            source.is_personal_domain,
            source.first_seen_at,
            source.last_seen_at,
            source.created_at,
            source.updated_at
          )
        """
        self._execute_logged_merge(
            query,
            self.settings.source_contact_table_fqn,
            self.settings.dealer_accounts_table_fqn,
        )

    def _merge_prospect_contacts(self) -> None:
        """Create or update canonical contact rows."""

        query = f"""
        MERGE `{self.settings.prospect_contacts_table_fqn}` AS target
        USING (
          SELECT
            TO_HEX(SHA256(email)) AS prospect_contact_id,
            ANY_VALUE(contact_id) AS source_contact_id,
            ANY_VALUE(full_name) AS full_name,
            ANY_VALUE(first_name) AS first_name,
            ANY_VALUE(last_name) AS last_name,
            email,
            email_domain,
            LOGICAL_OR(is_personal_email) AS is_personal_email,
            ANY_VALUE(domain_type) AS domain_type,
            CAST(NULL AS STRING) AS role_type,
            CAST(NULL AS STRING) AS role_title,
            ANY_VALUE(contact_status) AS contact_status,
            MAX(confidence_score) AS confidence_score,
            'contact_master_normalization' AS source_type,
            '{self.settings.source_contact_table}' AS source_table,
            MIN(observed_at) AS first_seen_at,
            MAX(observed_at) AS last_seen_at,
            CURRENT_TIMESTAMP() AS created_at,
            CURRENT_TIMESTAMP() AS updated_at
          FROM (
            SELECT
              contact_id,
              full_name,
              first_name,
              last_name,
              LOWER(TRIM(email)) AS email,
              LOWER(TRIM(email_domain)) AS email_domain,
              IFNULL(is_personal_email, FALSE) AS is_personal_email,
              domain_type,
              contact_status,
              IFNULL(confidence_score, 0.0) AS confidence_score,
              COALESCE(last_seen_at, created_at, CURRENT_TIMESTAMP()) AS observed_at
            FROM `{self.settings.source_contact_table_fqn}`
            WHERE email IS NOT NULL
              AND TRIM(email) != ''
          )
          GROUP BY email, email_domain
        ) AS source
        ON target.prospect_contact_id = source.prospect_contact_id
        WHEN MATCHED THEN
          UPDATE SET
            target.source_contact_id = COALESCE(target.source_contact_id, source.source_contact_id),
            target.full_name = COALESCE(target.full_name, source.full_name),
            target.first_name = COALESCE(target.first_name, source.first_name),
            target.last_name = COALESCE(target.last_name, source.last_name),
            target.email_domain = COALESCE(target.email_domain, source.email_domain),
            target.is_personal_email = IFNULL(target.is_personal_email, FALSE) OR source.is_personal_email,
            target.domain_type = COALESCE(target.domain_type, source.domain_type),
            target.contact_status = COALESCE(source.contact_status, target.contact_status),
            target.confidence_score = GREATEST(IFNULL(target.confidence_score, 0.0), IFNULL(source.confidence_score, 0.0)),
            target.source_type = source.source_type,
            target.source_table = source.source_table,
            target.first_seen_at = LEAST(IFNULL(target.first_seen_at, source.first_seen_at), source.first_seen_at),
            target.last_seen_at = GREATEST(IFNULL(target.last_seen_at, source.last_seen_at), source.last_seen_at),
            target.updated_at = CURRENT_TIMESTAMP()
        WHEN NOT MATCHED THEN
          INSERT (
            prospect_contact_id,
            source_contact_id,
            full_name,
            first_name,
            last_name,
            email,
            email_domain,
            is_personal_email,
            domain_type,
            role_type,
            role_title,
            contact_status,
            confidence_score,
            source_type,
            source_table,
            first_seen_at,
            last_seen_at,
            created_at,
            updated_at
          )
          VALUES (
            source.prospect_contact_id,
            source.source_contact_id,
            source.full_name,
            source.first_name,
            source.last_name,
            source.email,
            source.email_domain,
            source.is_personal_email,
            source.domain_type,
            source.role_type,
            source.role_title,
            source.contact_status,
            source.confidence_score,
            source.source_type,
            source.source_table,
            source.first_seen_at,
            source.last_seen_at,
            source.created_at,
            source.updated_at
          )
        """
        self._execute_logged_merge(
            query,
            self.settings.source_contact_table_fqn,
            self.settings.prospect_contacts_table_fqn,
        )

    def _merge_account_relationships(self) -> None:
        """Create or update account-to-contact relationship rows."""

        query = f"""
        MERGE `{self.settings.account_relationships_table_fqn}` AS target
        USING (
          SELECT
            TO_HEX(SHA256(CONCAT(
              account_key,
              '|',
              email
            ))) AS relationship_id,
            TO_HEX(SHA256(account_key)) AS dealer_account_id,
            TO_HEX(SHA256(email)) AS prospect_contact_id,
            'domain_membership' AS relationship_type,
            TRUE AS is_primary,
            'active' AS relationship_status,
            MAX(confidence_score) AS confidence_score,
            'contact_master_normalization' AS source_type,
            '{self.settings.source_contact_table}' AS source_table,
            MIN(observed_at) AS first_seen_at,
            MAX(observed_at) AS last_seen_at,
            CURRENT_TIMESTAMP() AS created_at,
            CURRENT_TIMESTAMP() AS updated_at
          FROM (
            SELECT
              LOWER(TRIM(account_key)) AS account_key,
              LOWER(TRIM(email)) AS email,
              IFNULL(confidence_score, 0.0) AS confidence_score,
              COALESCE(last_seen_at, created_at, CURRENT_TIMESTAMP()) AS observed_at
            FROM `{self.settings.source_contact_table_fqn}`
            WHERE account_key IS NOT NULL
              AND TRIM(account_key) != ''
              AND email IS NOT NULL
              AND TRIM(email) != ''
          )
          GROUP BY account_key, email
        ) AS source
        ON target.relationship_id = source.relationship_id
        WHEN MATCHED THEN
          UPDATE SET
            target.relationship_status = COALESCE(source.relationship_status, target.relationship_status),
            target.confidence_score = GREATEST(IFNULL(target.confidence_score, 0.0), IFNULL(source.confidence_score, 0.0)),
            target.last_seen_at = GREATEST(IFNULL(target.last_seen_at, source.last_seen_at), source.last_seen_at),
            target.updated_at = CURRENT_TIMESTAMP()
        WHEN NOT MATCHED THEN
          INSERT (
            relationship_id,
            dealer_account_id,
            prospect_contact_id,
            relationship_type,
            is_primary,
            relationship_status,
            confidence_score,
            source_type,
            source_table,
            first_seen_at,
            last_seen_at,
            created_at,
            updated_at
          )
          VALUES (
            source.relationship_id,
            source.dealer_account_id,
            source.prospect_contact_id,
            source.relationship_type,
            source.is_primary,
            source.relationship_status,
            source.confidence_score,
            source.source_type,
            source.source_table,
            source.first_seen_at,
            source.last_seen_at,
            source.created_at,
            source.updated_at
          )
        """
        self._execute_logged_merge(
            query,
            self.settings.source_contact_table_fqn,
            self.settings.account_relationships_table_fqn,
        )

    def _execute_logged_merge(self, query: str, source_table: str, target_table: str) -> None:
        """Execute a merge statement and log the result."""

        logger.info(
            "Running live write | source=%s | target=%s",
            source_table,
            target_table,
        )
        job = self.repository.execute_statement(query)
        logger.info(
            "Completed live write | source=%s | target=%s | affected_rows=%s",
            source_table,
            target_table,
            job.num_dml_affected_rows,
        )
